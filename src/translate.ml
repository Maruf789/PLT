...
let rec trans_stmts tid stmts = match stmts with
  ...
  | hd::tl -> ( match hd with
      ...
      | SCntFor (s, e, ss) -> 
          ...
          let isl, temparr = (trans_expr [] e) in
          let tt = ("TT") in
          ...
          in
          let fh = IForHead(fs1,
                          IBinop(IId iv, Lt, IBinop(ICall("rows",[IId tt]),Times,ICall("cols", [IId tt])), 
                          IAssign(IId iv, IBinop(IId iv, Plus, IIntval 1))
                         )
        in
          let mainbody =
             IAssign(IId s, IIndex(tt, IId iv))
             let lbody = trans_stmts (tid + 1) ss
            in
        in
