...
let rec trans_expr ttbl exp = match exp with
  ...
  | t, SMatval (ell, nr, nc) -> let tid = List.length ttbl in ttbl@[tid, ell], (sprintf " %s(T_%d, %d, %d) " (tpt t) tid nr nc)
and trans_arg_list ttbl sc el = match el with
    [] -> ""
  | [e] -> trans_expr ttbl e
  | e1::e2::tl ->  sprintf "%s %s %s" (trans_expr ttbl e1) sc (trans_arg_list sc (e2::tl))
...
and trans_stmts tid stmts = match stmts with
    [] -> []
  | hd::tl -> ( match hd with
        SEmpty -> [";"]
